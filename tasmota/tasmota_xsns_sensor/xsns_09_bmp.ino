...
#if defined(PROJECT_SPEC)
  const char kBmpTypes[] PROGMEM = "BMP180|BMP280|Sens|BME680";   //hawa: BME280 ==> Sens
#else
...
#if defined(PROJECT_SPEC)
              // hawa: sensName
              if (bmp_sensors[bmp_idx].bmp_address == 0x76) {
                snprintf_P(name, sizeof(name), PSTR("%s%c%s"), name, IndexSeparator(), "A");
              }
              if (bmp_sensors[bmp_idx].bmp_address == 0x77) {
                snprintf_P(name, sizeof(name), PSTR("%s%c%s"), name, IndexSeparator(), "I");
              }
#else
